...
router.post('/crawl', (req, res) => {
  ...
  ps.lookup({
    ...
    return res.status(200).json({
      err: 0, msg: 'Crawl task has been started', crawlList: { search, url }, process: { resultList },
    });
  });
  ...
  const scrape = async () => {
    ...
    try {
      while (await page.$('div.pagination__arrow._right') !== null) {
        ...
        await log.info('clicked on next page');
      }
    } catch (exception) {
      log.error(`EXCEPTION CAUGHT: ${exception}`);
    }
  ...
  scrape().then((value) => {
    ...
    try {
      fs.writeFile(`${homeDir}/${search.query}_${milliseconds}.json`, content, 'utf8', (err) => {
        if (err) {
          return log.error(err);
        }
        log.info(`JSON file was created at: ${homeDir}/${search.query}_${milliseconds}.json | Sending Email`);
        const mailOptions = {
          ...
          attachments: [
            {
              ...
              path: `${homeDir}/${search.query}_${milliseconds}.json`,
